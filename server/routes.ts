...
export async function registerRoutes(
  ...
  app.post("/api/scrape-url", async (req: any, res) => {
    try {
      ...
      const isBlockedSite = blockedHosts.some(h => hostname.includes(h));

      if (isBlockedSite) {
        return res.status(400).json({
          error: "This job site blocks automated access. Please use our Chrome extension to scan jobs on this site, or copy and paste the job details into the form below.",
          blocked: true,
        });
      }
      ...
      for (const ua of userAgents) {
        ...
        try {
          ...
          if (response.status !== 403 && response.status !== 429) {
            return res.status(400).json({
              error: `Could not access this page (status ${response.status}). Try copying and pasting the job details into the form instead.`,
            });
          }
        } catch (fetchErr: any) {
          clearTimeout(timeout);
          if (fetchErr.name === "AbortError") {
            return res.status(408).json({ error: "The request timed out. The site may be slow or blocking access. Try copying and pasting the job details into the form instead." });
          }
          return res.status(400).json({ error: `Could not reach the URL. Try copying and pasting the job details into the form instead.` });
        }
      }

      return res.status(400).json({
        error: "This site is blocking automated access. Please use our Chrome extension to scan jobs directly, or copy and paste the job details into the form below.",
        blocked: true,
      });
    } catch (err: any) {
      return res.status(500).json({ error: "Something went wrong. Try copying and pasting the job details into the form instead." });
    }
  });
