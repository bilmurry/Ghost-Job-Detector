...
import { agentAnalyze } from "./agentPipeline";
import { db } from "./db";
...
export async function registerRoutes(
  ...
  app.post("/api/analyze", async (req: any, res) => {
    ...
    try {
      ...
      try {
        const agentResult = await agentAnalyze(parseResult.data);
        result = agentResult.verdict;
        if (agentResult.looped) {
          console.log("[Agent] Follow-up loop completed");
        }
      } catch (err) {
        console.error("[Agent] Pipeline failed, falling back to rule engine:", err);
