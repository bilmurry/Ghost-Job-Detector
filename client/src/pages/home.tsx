...
function generatePdfReport(result: AnalysisResult, jobInfo?: { title?: string; company?: string }): void {
  const riskLabels: Record<string, string> = {
    high: "HIGH RISK - AVOID",
    medium: "MEDIUM RISK - PROCEED WITH CAUTION",
    "low-medium": "LOW-MEDIUM RISK - SOME CONCERNS",
...
${"-".repeat(30)}
Ghost Score: ${result.ghostScore}/100
Risk Level: ${riskLabels[result.riskLevel] || result.riskLevel.toUpperCase()}
...
CATEGORY BREAKDOWN
${"-".repeat(30)}
Content Analysis: ${result.detailedAnalysis.contentAnalysis.score}/100
  ${result.detailedAnalysis.contentAnalysis.flags.length > 0 ? result.detailedAnalysis.contentAnalysis.flags.map(f => `  - ${f}`).join("\n") : "  No issues found"}

Company Verification: ${result.detailedAnalysis.companyVerification.score}/100
  ${result.detailedAnalysis.companyVerification.flags.length > 0 ? result.detailedAnalysis.companyVerification.flags.map(f => `  - ${f}`).join("\n") : "  No issues found"}

Posting Patterns: ${result.detailedAnalysis.postingPatterns.score}/100
  ${result.detailedAnalysis.postingPatterns.flags.length > 0 ? result.detailedAnalysis.postingPatterns.flags.map(f => `  - ${f}`).join("\n") : "  No issues found"}

Communication: ${result.detailedAnalysis.communication.score}/100
  ${result.detailedAnalysis.communication.flags.length > 0 ? result.detailedAnalysis.communication.flags.map(f => `  - ${f}`).join("\n") : "  No issues found"}

`;

  if (result.redFlags.length > 0) {
    content += `DETECTED RED FLAGS
${"-".repeat(30)}
`;
...
`;
  ...
  content += `
${"=".repeat(50)}
Report generated by Ghost Job Detector
`;
  ...
  a.href = url;
  a.download = `ghost-job-report-${new Date().toISOString().split("T")[0]}.txt`;
...
function exportToJson(result: AnalysisResult, jobInfo?: { title?: string; company?: string }): void {
  const exportData = {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `ghost-job-analysis-${new Date().toISOString().split("T")[0]}.json`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
...
function ResultsDisplay({ result, jobInfo }: { result: AnalysisResult; jobInfo?: { title?: string; company?: string } }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => generatePdfReport(result, jobInfo)}
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => exportToJson(result, jobInfo)}
...
export default function Home() {
  ...
  const [analyzedJobInfo, setAnalyzedJobInfo] = useState<{ title?: string; company?: string } | null>(null);

  const analyzeMutation = useMutation({
    mutationFn: async (data: JobPosting) => {
      setAnalyzedJobInfo({ title: data.title, company: data.company });
